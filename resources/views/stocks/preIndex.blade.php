...
<table class="table-striped" id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-resizable="true" data-cookie="true" data-cookie-id-table="saveId" data-show-export="true" data-click-to-select="true" data-toolbar="#toolbar">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$stock->STOCK_AR_NAME}}</td>
            <td>{{$stock->STOCK_EN_NAME}}</td>
            ...
            <td>
                <div class="product-buttons">
                    <a href="{{ route('stocks.show',$stock->STOCK_ID)}}"><button title="View" class="pd-setting-ed"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                    <a href="{{ route('stocks.edit',$stock->STOCK_ID)}}"><button title="Edit" class="pd-setting-ed"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a>
                    <button data-toggle="modal" data-target="#del{{$stock->STOCK_ID}}" title="Trash" class="pd-setting-ed"><i class="fa fa-trash-o" aria-hidden="true"></i></button>
                </div>
         
        <!--Delete Company-->
       
        <div id="del{{$stock->STOCK_ID}}" class="modal modal-edu-general fullwidth-popup-InformationproModal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <h2>{{$stock->STOCK_AR_NAME}}</h2>
                     
                        <h4>هل تريد حذف جميع بيانات المخزن ؟ </h4>
                    </div>
                    <div class="modal-footer info-md">
                        <a data-dismiss="modal" href="#">إلغــاء</a>
                        <form id="delete" style="display: inline;" action="{{route('stocks.destroy',$stock->STOCK_ID)}}" method="POST">
                            @csrf
                            @method('DELETE')
