...
<div class="row">
    ...
    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
        <ul class="breadcome-menu">
            ...
            <li>
                <span class="bread-blod"> البنك </span>
            </li>
        </ul>
    </div>
</div>
                ...
                <div class="sparkline13-list">
                    <div class="sparkline13-hd">
                        <div class="main-sparkline13-hd">
                            <!-- <h1 style="direction:rtl"> بيانات البنوك </h1><br /> -->
                        </div>
                    </div>
                    <div class="sparkline13-graph">
                        <div class="datatable-dashv1-list custom-datatable-overright">
                            ...
                            <h3 style="text-align:right">بيانات البنوك</h3>
                            <a href="{{url('/Financial/Banks/Add')}}" class="btn_add_item">إضافة بنك</a>
                            <table class="table-striped" id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-resizable="false" data-cookie="true"
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th>كود البنك</th>
                                        <th>إسم البنك باللغة العربية</th>
                                        <th>إسم البنك باللغة الانجليزية</th>
                                        <th>عنوان البنك</th>
                                        <th>تليفون البنك</th>
                                        <th>فاكس البنك</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$Bank->branch_phone}}</td>
                                        <td>{{$Bank->branch_fax}}</td>
                                        <td>{{$Bank->open_balance}}جم</td>
                                        <td>{{date('Y-m-d', strtotime($Bank->balance_start_date))}}</td>
                                        <td>{{$Bank->current_balance}}جم</td>
                                        <td>{{$Bank->notes}}</td>
                                        <td>
                                            <div class="product-buttons">
                                                <a href="{{url("/Financial/Banks/View/$Bank->id")}}" class="pd-setting-ed btn"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                                <a href="{{url("/Financial/Banks/Edit/$Bank->id")}}" class="pd-setting-ed btn"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                            <a href="#" data-toggle="modal" data-target="#del{{$Bank->id}}" title="Trash" class="pd-setting-ed btn"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                            </div>
                                        </td>
                                    </tr>
                                    @endforeach
...
@endsection
@section('modal')
@foreach ($Banks as $BankModal)
    <!--Delete-->
<div id="del{{$BankModal->id}}" class="modal modal-edu-general fullwidth-popup-InformationproModal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <form action="{{url('/Financial/Banks/Delete')}}" method="post">
            {{ csrf_field() }}
            <div class="modal-body">
                <span class="educate-icon educate-danger modal-check-pro information-icon-pro"> </span>
                <h2>{{$BankModal->ar_name}}</h2>
                <h4>هل تريد حذف جميع بيانات البنك ؟  </h4>
                <input type="hidden" name="id" value="{{$BankModal->id}}">
            </div>
