...
<tr data-id="{{$counter}}">
    ...
    ?>
    <input type="text" id="upselectBatch{{$counter}}" readonly name="upselectBatch{{$counter}}" readonly value="{{$data->batch_no ?? ''}} /@if($dateBatch){{ date_format($dateBatch, 'Y-m-d')}}@endif /{{$data->item_total_qty ?? ''}}">
