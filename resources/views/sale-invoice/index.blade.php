	...
	<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
		<div class="breadcome-heading">
			<div class="add-product">
				<form action="{{route('sale-invoice.creation')}}" method="get">
					<input type="hidden" value="{{$row->id}}" id="branch" name="branch">

					<button type="submit" style="direction:ltr;margin-top:-20px;background: rgba(139,0,0,.7);color:white;padding:5px ; border-color: rgba(139,0,0,.7);">إضافة فاتورة مبيعات</button>
...
<script>
	$(document).ready(function() {
		...
		$('select[name="select_branch"]').on('change', function() {
			...
			$.ajax({
				url: "{{route('dynamicBranchSaleInvoice.fetch')}}",
				method: "get",
				data: {
					branch_id: branch,
